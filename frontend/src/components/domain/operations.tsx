...
export function RolePicker({ selectedRoles, onChange }: { selectedRoles: RoleCode[]; onChange: (roles: RoleCode[]) => void }) {
  return <fieldset><legend className="text-sm font-semibold text-gastro-primary">Roles</legend><p className="mt-1 text-sm text-gastro-muted">Selecciona uno o más roles para esta persona.</p><div className="mt-3 grid gap-2 sm:grid-cols-3">{(Object.keys(roles) as RoleCode[]).map((role) => { const selected = selectedRoles.includes(role); return <label className={cn("flex cursor-pointer items-center gap-2 rounded-lg border p-3 text-sm transition-colors", selected ? "border-gastro-action bg-orange-50" : "border-gastro-outline-variant bg-white hover:bg-gastro-surface-low")} key={role}><input checked={selected} className="size-4 rounded border-gastro-outline text-gastro-action focus:ring-gastro-action" onChange={() => onChange(selected ? selectedRoles.filter((current) => current !== role) : [...selectedRoles, role])} type="checkbox" />{roles[role]}</label>; })}</div></fieldset>;
}
